...
export default function ContactPage() {
  return (
    <>
      <PageHeader content={schoolConfig.pageContent.contact} />
      <section className="py-12 bg-white">
        <div className="container">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {schoolConfig.contact.contactCards.map((card, index) => {
            ...
            <Card className="bg-white">
              <CardContent className="p-6">
                <h2 className="text-xl font-extrabold text-slate-950">
                  {schoolConfig.contact.mapTitle}
                </h2>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  {schoolConfig.contact.mapDescription}
                </p>
                <a
                  href={schoolConfig.schoolInfo.googleMapsUrl}
                  target="_blank"
                  rel="noreferrer"
                ...
                >
                  <span className="min-w-0 flex-1 break-words">
                    {schoolConfig.schoolInfo.address}
                  </span>
